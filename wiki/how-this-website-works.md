...
Every 15 minutes a second Action diffs this repo against the last-synced commit (recorded in `.translation-sync` on both sides), translates new and changed pages into Chinese with the Kimi API (`kimi-k2.6`), mirrors deletions, pushes to [yelab-wiki-zh](https://gitee.com/ye-lab/yelab-wiki-zh), and immediately triggers the publishing sync (no second cron wait). End to end, an English edit typically reaches the Chinese site within ~20 minutes. Both workflows can also be run manually from the website repo's Actions tab.
